...
module "rds-oracle" {
  ...
  db_subnet_group_name   = try(aws_db_subnet_group.rds_subnet_group[0].name, "")
  subnet_ids             = [element(data.aws_subnets.database.ids, 0)]
  vpc_security_group_ids = [module.security_group_rds_oracle.security_group_id]
...
module "rds-postgresql" {
  ...
  create_db_parameter_group = var.create_postgresql
  # PostgreSQL은 option group을 사용하지 않음(2024.08.22)
  create_db_option_group = false

  identifier = "rds-${var.service}-${var.environment}-${var.rds_postgresql_name}"

  engine                          = var.rds_postgresql_engine
  engine_version                  = var.rds_postgresql_engine_version
  family                          = var.rds_postgresql_family               # DB parameter group
  major_engine_version            = var.rds_postgresql_major_engine_version # DB option group
  parameter_group_name            = "rdspg-${var.service}-${var.environment}-${var.rds_postgresql_name}"
  parameter_group_use_name_prefix = false
  parameter_group_description     = "parameter group for ${var.service}-${var.environment}-${var.rds_postgresql_name}"
  instance_class                  = var.rds_postgresql_instance_class

  storage_encrypted = true
  storage_type      = "gp3"
  # max_allocated_storage = var.rds_postgresql_allocated_storage * 1.1
  ...
  # "Error creating DB Instance: InvalidParameterValue: MasterUsername
  # user cannot be used as it is a reserved word used by the engine"
  db_name                     = var.rds_postgresql_db_name
  ...
  db_subnet_group_name   = try(aws_db_subnet_group.rds_subnet_group[0].name, "")
  subnet_ids             = [element(data.aws_subnets.database.ids, 0)]
  vpc_security_group_ids = [module.security_group_rds_postgresql.security_group_id]
  ...
  tags = merge(
    local.tags,
    {
      "Name" = "rds-${var.service}-${var.environment}-${var.rds_postgresql_name}"
    },
  )
}
