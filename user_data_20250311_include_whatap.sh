...
# Data영역 EBS 볼륨 마운트 (/data)
echo -e "n\np\n\n\n\nw\n" | fdisk /dev/nvme1n1           # fdisk로 새로운 파티션 생성
echo "fdisk new partition complete"
mkfs -t xfs /dev/nvme1n1p1                                  # mkfs로 xfs 파일시스템 포맷
echo "mkfs complete"
mkdir /data                                                 # /data 디렉터리 생성
mount /dev/nvme1n1p1 /data                                  # /data 디렉터리에 마운트
echo "mount complete"
UUID=$(blkid -s UUID -o value /dev/nvme1n1p1)               # UUID 추출
echo -e "UUID=$UUID\t/data\txfs\tdefaults\t0 2" >> /etc/fstab      # /etc/fstab에 Disk정보 추가

# Whatap 설치파일 S3에서 Download 및 install
ARCH=$(uname -m)
if [ "$ARCH" = "aarch64" ]; then     # x86 일 경우 x86_64 agent를 받아서 설치
...
elif [ "$ARCH" = "x86_64" ]; then    # ARM 일 경우 aarch64 agent를 받아서 설치
...
systemctl enable whatap-infra.service # Whatap Service Enable 및 테스트
