...
module "ec2_ogg" {
  ...
  # az_a를 따로 호출 (app subnet이 가용역영별 정렬이 되지 않을 수 있음)
  subnet_id                   = data.aws_subnets.app_vm_a.ids[0]
  vpc_security_group_ids      = [module.security_group_ec2_ogg.security_group_id]
  associate_public_ip_address = false
  disable_api_stop            = false
  disable_api_termination     = true
  # https://docs.aws.amazon.com/ko_kr/AWSEC2/latest/UserGuide/hibernating-prerequisites.html#hibernation-prereqs-supported-amis
  ...
  root_block_device = [
    {
      ...
      tags = merge(
        local.tags,
        {
          "Name" = "ebs-${var.service}-${var.environment}-ogg-root"
        },
      )
    },
  ]
  ...
  ebs_block_device = [
    {
      ...
      tags = merge(
        local.tags,
        {
          "Name" = "ebs-${var.service}-${var.environment}-ogg-data01"
        },
      )
    }
  ]
